...
export async function login(req: Request, res: Response) {
  ...
  try {
    ...
    res.json({
      token,
      user: { id: user.id, email: user.email, username: user.username, isOperator: user.isOperator },
    });
  } catch (err) {
...
export async function getMe(req: AuthRequest, res: Response) {
  try {
    ...
    res.json({ id: user.id, email: user.email, username: user.username, isOperator: user.isOperator });
  } catch (err) {
...
export async function requireOperator(req: AuthRequest, res: Response, next: NextFunction) {
  ...
  try {
    ...
    const [user] = await db.select({ isOperator: users.isOperator }).from(users).where(eq(users.id, decoded.userId));
    if (!user?.isOperator) {
