...
async function main() {
  ...
  // In Replit deployment builds, the web bundle is already produced by
  // `expo export --platform web --output-dir web-build` and the server
  // serves it directly. The metro-based asset manifest download below
  // can't reach a live deployment domain during build, so skip it.
  if (process.env.EXPO_PUBLIC_DEPLOYMENT_DOMAIN || process.env.REPLIT_DEPLOYMENT) {
    console.log("Deployment build detected — skipping legacy static-build (web-build/ already generated by expo export).");
    process.exit(0);
  }
